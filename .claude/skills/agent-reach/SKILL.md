...
This copy of the skill is vendored into the repo, but the `agent-reach` CLI and
its upstream tools are not — in an ephemeral container they are gone. If
`agent-reach` is not on `PATH`:
